...
초등 5~6학년이 공식 박물관 기록을 참고해 만든 학습용 재구성 이미지를 관찰하고, 새 단서에 따라 역사 가설을 유지·다듬기·바꾸기·판단 보류하며 기록하는 정적 교육 웹앱입니다.
...
- 사진 관찰 화면은 이미지 생성 모델로 만든 학습용 재구성 이미지를 로컬 정적 자산으로 사용합니다. 실제 소장품 사진으로 오해하지 않도록 화면과 출처 대화상자에 명시합니다.
- 앱에는 원격 이미지 요청, 로그인, API, 저장소, 업로드, 자유 입력, 런타임 AI 기능이 없습니다. 이미지는 빌드 전에 생성해 정적 파일로 포함합니다.
- 생성 이미지의 관찰 특징과 참고한 공식 기록, SHA-256, 생성·처리 내역은 [이미지 권리 원장](docs/image-rights-ledger.md)에 있습니다.
