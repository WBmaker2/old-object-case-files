...
const supportText = {
  "best-supported": "새 자료까지 보면 이 생각이 가장 잘 맞아요.",
  "plausible-limited": "가능한 생각이지만, 더 많은 자료가 필요해요.",
  "conflicts-with-clue": "새 자료와 맞지 않는 부분이 있어요.",
  "responsible-defer": "지금은 아직 결정하지 않아도 괜찮아요. (판단 보류)",
};
...
export function CasePanel({ caseFile, onComplete }: { caseFile: CaseFile; onComplete: (versions: HypothesisVersion[]) => void }) {
  ...
  function saveInitial(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setVersions([version]);
      setMessage("첫 생각(가설 1)을 기록했어요. 다음에는 박물관이 적은 내용을 읽어요.");
      setStep("catalog");
    } catch (error) {
  ...
  function recordRevision(stage: 2 | 3, action: RevisionAction, hypothesisId: string, evidenceIds: string[]) {
    try {
      ...
      setMessage(`생각 ${stage}를 기록했어요. 앞의 기록은 그대로 남아 있어요.`);
  ...
  return (
    <section aria-labelledby="case-title" className="case-panel">
      ...
      {step === "observe" && <div className="case-step">
        ...
        <button className="button" disabled={observations.length !== 2} onClick={() => setStep("initial")} type="button">사진을 보고 첫 생각 고르기</button>
      </div>}

      {step === "initial" && <form className="case-step" onSubmit={saveInitial}>
        <h2 ref={headingRef} tabIndex={-1}>가설 1 · 지금의 생각</h2>
        <p className="case-action">사진과 가장 잘 맞는 생각을 하나 고르세요. 가설은 단서를 보고 세운 지금의 생각이에요.</p>
        <fieldset>
          <legend>사진과 가장 잘 맞는 생각을 고르세요</legend>
        ...
        <p className="gentle-note">처음 생각을 바꾸어도 틀린 것이 아니에요.</p>
        {!initialHypothesis && <p className="selection-guide">생각 하나를 고르면 기록할 수 있어요.</p>}
        <button className="button" disabled={!initialHypothesis} type="submit">첫 생각 기록하기</button>
      </form>}

      {step === "catalog" && <div className="case-step">
        <h2 ref={headingRef} tabIndex={-1}>박물관 기록을 읽어 봐요</h2>
        <p className="case-action">박물관이 이 물건에 대해 적어 둔 내용을 읽어 보세요.</p>
        {clueCard(2)}
        <button className="button" onClick={() => setStep("revision")} type="button">새 자료로 내 생각 고치기</button>
      </div>}

      {step === "revision" && <RevisionForm caseFile={caseFile} heading="가설 2 · 목록 단서 뒤의 생각" headingRef={headingRef} onSave={(action, hypothesis, evidence) => recordRevision(2, action, hypothesis, evidence)} previous={versions[0]} stage={2} studentHeading="박물관 기록을 보고 생각 고치기" />}

      {step === "context" && <div className="case-step">
        <h2 ref={headingRef} tabIndex={-1}>다른 자료를 함께 읽어 봐요</h2>
        <p className="case-action">다른 자료와 함께 살펴보고 마지막 생각을 준비하세요.</p>
        {clueCard(3)}
        <button className="button" onClick={() => setStep("final")} type="button">마지막 생각 고르기</button>
      </div>}

      {step === "final" && <RevisionForm caseFile={caseFile} heading="가설 3 · 지금의 가장 조심스러운 설명" headingRef={headingRef} onSave={(action, hypothesis, evidence) => recordRevision(3, action, hypothesis, evidence)} previous={versions[1]} stage={3} studentHeading="마지막 생각을 고르기" />}

      {step === "summary" && <div className="case-step">
        <h2 ref={headingRef} tabIndex={-1}>사건 정리</h2>
        <p className="case-action">내가 고른 생각과 아직 모르는 점을 짧게 살펴보세요.</p>
        ...
        <p className="final-reading"><strong>내 마지막 생각:</strong> {finalVersion?.statement} <br />{finalHypothesis && supportText[finalHypothesis.supportByStage[3]]} {finalVersion?.action === "refine" ? "" : finalHypothesis?.scopeLimit}</p>
